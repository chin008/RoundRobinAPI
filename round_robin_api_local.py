...
import urllib.request, urllib.error, json, threading, time, os

BACKENDS = os.getenv("BACKENDS", "http://backend1:5001,http://backend2:5002,http://backend3:5003").split(",")
    ...
    while True:
        for backend in BACKENDS:
            if not backend_status[backend]["healthy"]:
                try:
                    req = urllib.request.Request(backend, data=b"{}", headers={"Content-Type": "application/json"}, method="POST")
                    with urllib.request.urlopen(req, timeout=2) as resp:
                        if resp.status == 200:
                            backend_status[backend]["healthy"] = True
                            backend_status[backend]["failures"] = 0
                            print(f"[HEALTH] {backend} recovered")
                except Exception:
                    pass
    ...
    def do_POST(self):
        ...
        while attempts < len(BACKENDS):
            ...
            try:
                ...
                req = urllib.request.Request(backend, data=post_data, headers={"Content-Type": "application/json"}, method="POST")
                ...
                print(f"[ERROR] {backend} failed ({backend_status[backend]['failures']}): {e}")
                if backend_status[backend]["failures"] >= FAILURE_THRESHOLD:
                    backend_status[backend]["healthy"] = False
                    print(f"[HEALTH] Marking {backend} as UNHEALTHY")
